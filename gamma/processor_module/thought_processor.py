...
class APITransformer:
    def __init__(self, api_key, model="qwen-plus"):  # 可根据需要更改默认模型
        # 使用传入的API密钥，而不是硬编码
        ...
        2. 识别关键技术概念和评估标准  
        3. 生成技术思考摘要（50字内）
        4. 预测可能的后续技术问题（具体的问题，不是概述）
        5. 如果是非技术问题，返回{"status": "reject", "reason": "非技术问题"}

        重要：必须输出严格的JSON格式，包含所有字段：
        {
          "keywords": ["关键词1", "关键词2"],
          "concepts": ["概念1", "概念2"], 
          "summary": "技术思考摘要",
          "prediction": "基于当前问题，面试官可能会继续问：具体的后续问题",
          "status": "process"
        }
        ...
        - 必须是具体的技术问题，不是泛泛而谈
        - 基于当前问题的技术领域进行深入
        - 格式示例："面试官可能会继续问：如何在分布式环境下保证数据一致性？"
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'AbstractText' in data and data['AbstractText']:
                    results.append(f"概述: {data['AbstractText']}")
                ...
                search_info = " | ".join(results) if results else "未找到相关信息"
                logger.info(f"搜索完成，获得 {len(results)} 条结果")
                return search_info[:1000]  # 限制长度
            else:
                logger.warning(f"搜索API返回状态码: {response.status_code}")
                return "搜索服务暂时不可用"
    ...
    def generate_deep_answer(self, question, search_info=""):
        """生成深度回答（流式输出）"""
        try:
            logger.info(f"开始生成深度回答: {question[:50]}...")
            
            deep_prompt = f"""
            你是一个专业的技术专家，请对以下技术问题进行深度分析和详细解答。

            问题：{question}

            搜索到的最新信息：{search_info}
            ...
            stream = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是一个专业的技术专家，擅长深度分析技术问题并提供详细的结构化回答。"},
            ...
            logger.info("开始接收流式回答...")
            return stream
            
        except Exception as e:
            logger.error(f"生成深度回答失败: {str(e)}")
            traceback.print_exc()
            return None
    ...
    def extract_info_from_response(self, content):
        ...
        if "concepts" in content:
            try:
                start = content.index("concepts") + len("concepts") + 3
                end = content.index("]", start)
                concepts_str = content[start:end]
                result["concepts"] = [c.strip('" ') for c in concepts_str.split(",")]
            except:
                pass
        ...
        if "summary" in content:
            try:
                start = content.index('"summary"') + len('"summary"') + 2
                end = content.index('"', start)
        ...
        if "prediction" in content:
            try:
                start = content.index('"prediction"') + len('"prediction"') + 2
                end = content.index('"', start)
                result["prediction"] = content[start:end]
            except:
                try:
                    # 备用方案：查找prediction后的内容
                    start = content.index("prediction") + len("prediction") + 3
                    lines = content[start:].split('\n')
                    for line in lines:
                        if line.strip() and not line.strip().startswith('"') and '"' in line:
                            result["prediction"] = line.strip().strip('"').strip(',')
                            break
                except:
                    pass
...
class TechnicalThoughtProcessor:
    def __init__(self, api_transformer):
        ...
        # 深度思考相关
        self.deep_thinking_active = False
        self.deep_answer_content = ""
        self.deep_thinking_status = "待机中"
        self.deep_stream_thread = None
        self.deep_answer_lock = threading.Lock()
    ...
    def _update_thought(self, result, question_text):
        ...
        if "prediction" in result and result['prediction']:
            ...
            logger.info(f"添加预测: {result['prediction'][:50]}...")
        else:
            logger.warning(f"预测字段为空或不存在: {result.get('prediction', 'None')}")
    ...
    def start_deep_thinking(self, question):
        """启动深度思考"""
        try:
            if self.deep_thinking_active:
                logger.warning("深度思考已在进行中")
                return False
                
            logger.info(f"启动深度思考: {question[:50]}...")
            self.deep_thinking_active = True
            self.deep_answer_content = ""
            self.deep_thinking_status = "🔍 联网搜索中..."
            
            # 创建深度思考线程
            self.deep_stream_thread = threading.Thread(
                target=self._deep_thinking_worker,
                args=(question,),
                daemon=True
            )
            self.deep_stream_thread.start()
            
            return True
            
        except Exception as e:
            logger.error(f"启动深度思考失败: {str(e)}")
            self.deep_thinking_active = False
            return False

    def _deep_thinking_worker(self, question):
        """深度思考工作线程"""
        try:
            # 第一步：联网搜索
            logger.info("第一步：开始联网搜索...")
            with self.deep_answer_lock:
                self.deep_thinking_status = "🔍 联网搜索中..."
            
            search_info = self.api_transformer.search_web(question)
            logger.info(f"搜索完成，结果长度: {len(search_info)}")
            
            # 第二步：生成深度回答
            logger.info("第二步：开始生成深度回答...")
            with self.deep_answer_lock:
                self.deep_thinking_status = "🤖 AI正在深度分析..."
            
            stream = self.api_transformer.generate_deep_answer(question, search_info)
            
            if stream:
                logger.info("开始处理流式回答...")
                content_buffer = ""
                
                for chunk in stream:
                    if not self.deep_thinking_active:
                        logger.info("深度思考被用户中断")
                        break
                    ...
                    if chunk.choices and chunk.choices[0].delta and chunk.choices[0].delta.content:
                        ...
                        # 更新内容
                        with self.deep_answer_lock:
                            self.deep_answer_content = content_buffer
                            self.deep_thinking_status = "📝 正在生成回答..."
                ...
                # 完成处理
                with self.deep_answer_lock:
                    self.deep_thinking_status = "✅ 深度分析完成"
                    
                logger.info(f"深度思考完成，总字数: {len(content_buffer)}")
            else:
                logger.error("无法获取流式回答")
                with self.deep_answer_lock:
                    self.deep_answer_content = "抱歉，生成深度回答时出现错误。"
                    self.deep_thinking_status = "❌ 生成失败"
                    
        except Exception as e:
            logger.error(f"深度思考工作线程错误: {str(e)}")
            traceback.print_exc()
            with self.deep_answer_lock:
                self.deep_answer_content = f"深度思考过程中出现错误: {str(e)}"
                self.deep_thinking_status = "❌ 处理错误"
        finally:
            self.deep_thinking_active = False

    def get_deep_answer_stream(self):
        """获取深度回答的流式内容"""
        with self.deep_answer_lock:
            return {
                "content": self.deep_answer_content,
                "status": self.deep_thinking_status,
                "finished": not self.deep_thinking_active,
                "length": len(self.deep_answer_content)
            }

    def stop_deep_thinking(self):
        """停止深度思考"""
        try:
            logger.info("用户请求停止深度思考")
            self.deep_thinking_active = False
            
            with self.deep_answer_lock:
                self.deep_thinking_status = "🛑 已停止"
                
            if self.deep_stream_thread and self.deep_stream_thread.is_alive():
                # 等待线程结束
                self.deep_stream_thread.join(timeout=2)
                
            return True
            
        except Exception as e:
            logger.error(f"停止深度思考失败: {str(e)}")
            return False
    ...
    def get_processor_status(self):
        ...
        return {
            ...
            "deep_thinking_active": self.deep_thinking_active
        }
